    ...
    else
    {
        ...
        */
        ...
        if ((windProperties = json_object_dotget_object(rootObject, "wind")) != NULL)
        {
            ...
            {
                telemetry->latest.weather.wind_speed = (float)json_object_get_number(windProperties, "speed");
            }

            if (json_object_has_value_of_type(windProperties, "deg", JSONNumber))
            {
                telemetry->latest.weather.wind_direction = (int)json_object_get_number(windProperties, "deg");
            }
        }
...
float get_float_by_key(JSON_Object *properties, const char *key)
{
    ...
    {
        result = (float)json_object_get_number(properties, key);
    }
