...
static void device_twin_update_float(float *latest_value, float *previous_value, DX_DEVICE_TWIN_BINDING *device_twin)
{
    if (*latest_value != *previous_value) {
        *previous_value = *latest_value;
        dx_deviceTwinReportValue(device_twin, latest_value);
...
void publish_telemetry(ENVIRONMENT_TELEMETRY *environment)
{
    ...
    size_t msg_len =
        (size_t)snprintf(msgBuffer, sizeof(msgBuffer), publish_template, environment->latest.weather.temperature,
                 ...
                 environment->latest.pollution.sulphur_dioxide, environment->latest.pollution.ammonia, environment->latest.pollution.pm2_5,
