...
static void handleRecvMsg(void *handle)
{
    ...
    case DISK_IC_READ:
        ...
        entry = find_in_cache((uint32_t)(pMsg->sector_number + (pMsg->drive_number * 3000)));
        if (entry == NULL)
        {
        ...
        {
            memcpy(pMsg->sector, entry->sector, 137);
            pMsg->disk_ic_msg_type = DISK_IC_READ;
            pMsg->success = true;
            WriteDataToA7(pMsg, sizeof(INTERCORE_DISK_DATA_BLOCK_T));
        }
    ...
    case DISK_IC_WRITE:
        ...
        if (pMsg->success){
            add_to_cache((uint32_t)(pMsg->sector_number + (pMsg->drive_number * 3000)), pMsg->sector);
        }
